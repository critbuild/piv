...
pub enum ControlCommand {
    ...
    Highlight { path: PathBuf, start_line: usize, end_line: usize },
    Line(usize),
    TabNext,
    TabPrev,
    Recenter,
    ...
    match command {
        ...
        ControlCommand::Highlight { path, start_line, end_line } => {
            if start_line == end_line {
                format!("highlight {}:{}", path.display(), start_line)
            } else {
                format!("highlight {}:{}-{}", path.display(), start_line, end_line)
            }
        }
...
pub fn parse_control_message(line: &str) -> Option<ControlCommand> {
    ...
    if let Some(rest) = line.strip_prefix("highlight ") { return crate::cli::parse_highlight_target(rest).ok(); }
